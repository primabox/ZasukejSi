...
@php
    $advertFeatures = [
        [
            'icon' => 'hand-heart',
            'iconClass' => 'w-8 h-8',
            'text' => 'Oprávněné aniž i osoby vede grafikou osobami úmyslu 60 %',
        ],
        [
            'icon' => 'icecream',
            'iconClass' => 'w-8 h-8',
            'text' => 'Oprávněné aniž i o snadno osoby grafikou osobami úmyslu 60 %',
        ],
        [
            'icon' => 'laptop',
            'iconClass' => 'w-9 h-9',
            'text' => 'Oprávněné aniž i o snadno osoby osobami úmyslu 60 %',
        ],
    ];
@endphp
    ...
    <div class="lg:hidden">
        ...
        <div class="mx-auto mt-5 grid w-full max-w-[346px] grid-cols-1 gap-4">
            @foreach ($advertFeatures as $feature)
                <div class="flex items-center gap-5 rounded-[12px] bg-white px-6 py-5 shadow-[0_10px_24px_rgba(92,45,98,0.10)]">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center text-primary">
                        <x-icons :name="$feature['icon']" :strokeWidth="2.5" :class="$feature['iconClass']" />
                    </div>
                ...
                @foreach ($advertFeatures as $feature)
                    <div class="feature-block">
                        <div class="feature-block-icon">
                            <x-icons :name="$feature['icon']" :strokeWidth="2.5" :class="$feature['iconClass'] . ' text-primary'" />
                        </div>
